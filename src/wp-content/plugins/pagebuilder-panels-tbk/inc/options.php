...
function siteorigin_panels_options_admin_menu() {
	add_options_page( __('Page Builder', 'so-panels'), __('Page Builder', 'so-panels'), 'manage_options', 'siteorigin_panels', 'siteorigin_panels_options_page' );
}
add_action( 'admin_menu', 'siteorigin_panels_options_admin_menu' );

function siteorigin_panels_options_page(){
	include plugin_dir_path(__DIR__).'tpl/options.php';
}
...
function siteorigin_panels_options_init() {
	...
	add_settings_field( 'bootstrap-ver', __('Bootstrap Version', 'so-panels'), 'siteorigin_panels_options_field_display', 'siteorigin-panels', 'display', array( 'type' => 'bootstrap-ver' ));
...
function siteorigin_panels_options_field_display($args){
	...
	switch($args['type']) {
		...
		case 'bootstrap-ver' :
			?>
			<select name="siteorigin_panels_display[<?php echo esc_attr($args['type']) ?>]">
				<option <?php echo selected($settings[$args['type']], '<3');?>><3</option>
				<option <?php echo selected($settings[$args['type']], '>3');?>>>3</option>
			</select>
			<?php
			break;
		...
		case 'margin-sides' :
		case 'mobile-width' :
			?><input type="text" name="siteorigin_panels_display[<?php echo esc_attr($args['type']) ?>]" value="<?php echo esc_attr($settings[$args['type']]) ?>" class="small" /><?php
			break;
	}
}
