...
Version: 1.1.5
Author: SiteOrigin/tbk Creative
...
function siteorigin_panels_setting($key = false){
	...
	if(empty($settings)){
		...
		$settings = wp_parse_args($settings, array(
			...
			'bootstrap-ver' => !isset($display_settings['bootstrap-ver']) ? 3 : $display_settings['bootstrap-ver'],
...
function siteorigin_panels_save_post( $post_id ) {
	...
	update_post_meta( $post_id, 'section-classes', $_POST['section-classes']);
...
function siteorigin_panels_css() {
	...
	foreach ( $panels_data['grids'] as $gi => $grid ) {
		...
		for ( $i = 0; $i < $cell_count; $i++ ) {
			...
			if ( $cell_count > 1 ) {
				if(maybe_custom_column(round( $cell['weight'] * 100, 3 ))) {
					$css_new = 'width:' . round( ($cell['weight'] * 100 - ($settings['margin-sides']/2)), 3 ) . '%';
...
function siteorigin_panels_render( $post_id = false ) {
	...
	$display_settings = get_option('siteorigin_panels_display');
	ob_start();
	foreach ( $grids as $gi => $cells ) {?>
		<section class="panel-grid <?php echo ($panels_data['grids'][$gi]['classes']);?>" id="pg-<?php echo $gi ?>">
			<div class="container">
				<div class="row<?php echo (isset($display_settings['responsive']) && $display_settings['responsive'] !== false?'-fluid':'');?>">
					<?php foreach ( $cells as $ci => $widgets ) {?>
						<div class="<?php echo bootstrap_columns(count($cells), $display_settings['bootstrap-ver']);?>" id="pgc-<?php echo $gi . '-' . $ci ?>">
							<?php
...
function bootstrap_columns($total, $ver) {
	$grid = 12;
	$col = $grid / $total;
	if($ver == '>3') {
		return 'col-lg-'.$col.' col-md-'.$col;
	}
	return 'span'.$col;
}

function maybe_custom_column($col){
	//decide if we've modified the column sizes
	$columns = array(25, 33.333, 50, 75, 100);
...
<?php //BEGIN::SELF_HOSTED_PLUGIN_MOD
	...
	**********************************************/
	...
	$__UpdateChecker = new PluginUpdateChecker('http://plugins.tbkcreative.com/extend/plugins/pagebuilder-panels-tbk/update', __FILE__,'pagebuilder-panels-tbk');			
