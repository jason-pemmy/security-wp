...
class PluginUpdateChecker {
	...
	function requestInfo($queryArgs = array()){
		...
		// Give the option of doing this differently
		if (!$result = apply_filters('puc_get_plugin_info-'.$this->slug,false)){
	...
	/**
	 * If an upgrade notice exists, print it
	 * 
	 * @return void
	 */
	function maybeInjectUpgradeNotice($plugin_data,$r){
		if (is_string($r->upgrade_notice) and $r->upgrade_notice != ''){
			printf( '<br/>'.$r->upgrade_notice );			
		}
		elseif(is_a($r->upgrade_notice,'stdClass') and isset($r->new_version) and isset($r->upgrade_notice->{$r->new_version})){
			printf( '<br/>'.$r->upgrade_notice->{$r->new_version} );			
		}
	}
